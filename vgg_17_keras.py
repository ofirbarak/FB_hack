...
def base_model():
    ...
    model.add(Conv2D(256, (3, 3), padding='same', name='block2_conv2'))
    model.add(BatchNormalization()) if BATCH_NORM else None
    model.add(Activation('relu'))
...
def load_model_and_weights(weights_path="model.h5"):
    model = base_model()
...
def run(model, image):
    # resize image
    resized_im = cv2.resize(image, (224, 224)).astype(np.float32)
    resized_im = resized_im.transpose((2,0,1))
    resized_im = np.expand_dims(resized_im, axis=0)

    predict = model.predict(resized_im)
